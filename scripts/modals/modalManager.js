(function initModalManager(global) {
  class ModalManager {
    ...
    cacheDomReferences() {
      ...
      this.modalCard = this.modalOverlay ? this.modalOverlay.querySelector('.modal__content') : null;

      if (!this.modalOverlay) {
        return;
      }

      this.modalTitleElement = this.modalOverlay.querySelector('.modal__title');
      this.modalDescriptionElement = this.modalOverlay.querySelector('.modal__description');
      this.modalVideoIframe = this.modalOverlay.querySelector('.modal__video-container iframe');
      this.modalImageElement = this.modalOverlay.querySelector('#modal-image-element');
      this.modalVideoContainer = this.modalOverlay.querySelector('.modal__video-container');
      this.modalHoverImageElement = this.modalOverlay.querySelector('#modal-hover-image');
      this.modalGalleryElement = this.modalOverlay.querySelector('.modal__gallery');
      this.modalBadgesContainer = this.modalOverlay.querySelector('.modal__badges');
      this.modalButtonsContainer = this.modalOverlay.querySelector('.modal__actions');
    }
    ...
    handleBodyClick(event) {
      const triggerButton = event.target.closest('.highlight-project .btn, .project-modal-trigger, .game-card');
      if (!triggerButton) {
        return;
      }
    ...
    swipeInteractionsEnabled() {
      if (!this.modalOverlay || !this.modalOverlay.classList.contains('modal--active')) {
        return false;
      }
      ...
      data.badges.forEach((badgeData) => {
        ...
        if (badgeData.className) {
          ...
          classNames.filter(Boolean).forEach((cls) => {
            // Mapping specific known classes if necessary
            if (cls === 'badge-award') {
                badgeEl.classList.add('badge--award');
            } else {
    ...
    populateGallery(data) {
      ...
      data.gallery.forEach((item, index) => {
        ...
        imgEl.alt = altText;
        imgEl.className = 'modal__gallery-img';
    ...
    populateButtons(data) {
      ...
      if (data.playUrl) {
        ...
        playButton.className = 'btn modal__play-btn';
        ...
        playMessage.className = 'modal__note';
